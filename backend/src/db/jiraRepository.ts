...
export async function upsertJiraSnapshot(projects: JiraProject[], issues: JiraIssue[]) {
  await withDatabaseConnection(async (connection) => {
    ...
    try {
      ...
      for (const issue of issues) {
        if (!issue.project?.key || !issue.created || !issue.updated) continue;
        await connection.execute(
      ...
      await connection.execute(
        `INSERT INTO jira_sync_state (source_name, last_synced_at, last_success_at, last_error)
...
export async function getStoredSnapshot(filters: { projectKey?: string; issueType?: string; startDate?: string; endDate?: string } = {}) {
  return withDatabaseConnection(async (connection) => {
    ...
    if (!filters.startDate && !filters.endDate) clauses.push('i.created_at >= UTC_TIMESTAMP() - INTERVAL 30 DAY');

    const [rows] = await connection.execute<RowDataPacket[]>(
      `SELECT id, issue_key, project_key, summary, status, issue_type, priority, created_at, updated_at, resolved_at
       FROM jira_issues i WHERE ${clauses.join(' AND ')} ORDER BY i.created_at DESC LIMIT 5000`,
      values,
    );
    const issues: JiraIssue[] = rows.map((row) => ({
...
export async function getSyncState() {
  return withDatabaseConnection(async (connection) => {
    const [rows] = await connection.query<RowDataPacket[]>('SELECT source_name, last_synced_at, last_success_at, last_error FROM jira_sync_state WHERE source_name = \'jira\'');
    return rows[0] || null;
  });
}
